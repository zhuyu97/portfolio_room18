...
public interface StockDetailDao extends JpaRepository<StockDetail, Long> {
    ...
    @Query(value = "SELECT stock_price FROM stock_detail WHERE stock_id = :stockId AND DATE(time) = CURDATE() ORDER BY `time` ASC",nativeQuery = true)
    public List<BigDecimal> getTodayPricesByStockId(@Param("stockId") Long stockId);
}